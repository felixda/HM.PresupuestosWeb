...
public class ThemeJsChangeDispatcher : ComponentBase, IThemeChangeRequestDispatcher, IAsyncDisposable {
    ...
    protected override void OnInitialized()
    {
        base.OnInitialized();
        if (Themes == null)
            throw new InvalidOperationException("Themes service is not provided.");
    ...
    public async void RequestThemeChange(Theme theme)
    {
        ...
        _pendingTheme = theme;
        if (_module == null || Themes == null)
            return; // Prevent null reference usage
    ...
    public async Task ThemeLoadedAsync()
    {
        if (_pendingTheme == null)
            return;
        if (Themes != null && Themes.ThemeLoadNotifier != null)
        {
