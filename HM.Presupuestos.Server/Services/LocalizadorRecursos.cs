...
namespace HM.Presupuestos.Server.Services
{
    ...
    public class LocalizadorRecursos : ILocalizadorRecursos
    {
            ...
            IServiceScopeFactory scopeFactory, IProveedorRecursosJson proveedorJson)
        {
            ...
            {
                Console.WriteLine("[LocalizadorRecursos] ?? Idioma cambiado, limpiando caché de valores...");
            ...
            try
            {
                ...
                foreach (var languageCode in languageCodeList)
                {
                    try
                    {
                        ...
                        var languageItem = new Idioma { Iso = languageCode };
                    ...
                    {
                        Console.WriteLine($"[LocalizadorRecursos] ? Error procesando idioma {languageCode}: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
