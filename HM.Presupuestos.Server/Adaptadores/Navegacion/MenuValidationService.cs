using HM.Presupuestos.Server.Adaptadores.Idioma;
...
namespace HM.Presupuestos.Server.Adaptadores.Navegacion
{
    ...
    public class ValidadorMenusUsuario : IValidadorMenusUsuario
    {
        ...
        public async Task<List<ResultadoValidacionSubmenu>> ValidarSubmenusDe(UsuarioEntidad usuario)
        {
            ...
            {
                _logger.LogWarning("[MenuValidation] ?? Usuario sin menús para validar");
                return resultados;
            }
            ...
            {
                _logger.LogWarning($"[MenuValidation] ?? Usuario {usuario.Login} no tiene menús hijos");
                return resultados;
            }
            ...
            foreach (var menu in menusHijos)
            {
                ...
                try
                {
                    ...
                    if (string.IsNullOrWhiteSpace(url))
                    {
                        ...
                        resultado.Mensaje = "?? URL vacía o no encontrada en recursos";
                        _logger.LogWarning($"[MenuValidation] ?? Menú hijo {menu.Id} ({menu.NombreMenu}) sin URL");
                    }
                    else
                    {
            ...
            if (menusInvalidos > 0)
            {
                _logger.LogWarning($"[MenuValidation] ?? Menús hijos con URLs inválidas:");
