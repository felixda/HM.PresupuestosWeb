...
El sistema SHALL mostrar una página de administración accesible solo para usuarios con permiso `CodigosMenu.UsuariosConectados` que lista todas las sesiones SSO activas en ese momento, con las columnas: Login, Página actual, Conectado desde (hora UTC), Tiempo en sesión (calculado).
...
El sistema SHALL permitir actualizar la lista de sesiones activas mediante un botón explícito. No SHALL actualizarse automáticamente de forma periódica.

#### Scenario: Refrescar la lista

- **WHEN** el administrador pulsa el botón "Actualizar"
- **THEN** el sistema recarga la lista con el estado actual de sesiones
...
El sistema SHALL actualizar la página actual registrada para una sesión activa cada vez que el usuario navega a una nueva ruta.

#### Scenario: Navegación a nueva página

- **WHEN** un usuario SSO autenticado navega a una nueva URL dentro de la aplicación
- **THEN** el sistema actualiza la página actual de su sesión en el registro

#### Scenario: Usuario no autenticado navega

- **WHEN** un usuario no autenticado (sin sesión registrada) navega
- **THEN** el sistema no realiza ninguna actualización en el registro
